...
function cc_dynamicSliderMaker() {
  const c = document.querySelectorAll(".splide");
  if (c.length < 1) return;
  const n = [],
    e = [],
    i = [];
  c.forEach(c => {
    const d = c.getAttribute("data-slide-sync"),
      o = c.getAttribute("id");
    if (!window["cc-" + o]) {
      if (window["cc-" + o] = new Splide(c), n.includes(o)) return;
      d ? (e.push({
        primary: o,
        secondary: d
      }), n.push(d)) : i.push(o)
    ...
    i.includes(c) && i.splice(i.indexOf(c), 1), i.includes(n) && i.splice(i.indexOf(n), 1)
  }), e.forEach(({
    primary: c,
    secondary: n
  }) => {
    if (window["cc-" + c] && window["cc-" + n]) {
      const e = document.querySelector(`#${c}`).getAttribute("data-splide"),
        i = JSON.parse(e);
      window["cc-" + c].sync(window["cc-" + n]), i.isNavigation ? (window["cc-" + n].mount(), window["cc-" + c].mount()) : (window["cc-" + c].mount(), window["cc-" + n].mount())
    } else i.push(c), i.push(n)
  }), i.forEach(c => {
    window["cc-" + c] && window["cc-" + c].mount()
  })
}
document.addEventListener("DOMContentLoaded", function() {
  cc_dynamicSliderMaker()
});