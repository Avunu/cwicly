...
class Main_Query_API extends \WP_REST_Posts_Controller {
	...
	public function build_query( $query_args, $query_type, $query_block_id ) {
		...
		if ( 'posts' === $query_type ) {
			...
			// // Check if this is a search query and Relevanssi is active
			// if ( isset( $query_args['s'] ) && ! empty( $query_args['s'] ) && function_exists( 'relevanssi_do_query' ) ) {
			// 	// $query = new \WP_Query( $query_args );
			// 	// Hand over the search query to Relevanssi
			// 	// relevanssi_do_query( $query );
				
			// 	// Log Relevanssi results for debugging
			// 	if ( defined('WP_DEBUG') && WP_DEBUG ) {
			// 		$search_term = sanitize_text_field( $query_args['s'] );
			// 		$post_ids = array();
			// 		$post_titles = array();
			...
			// 		// Create a specific log file in wp-content
			// 		$log_file = WP_CONTENT_DIR . '/query_api_debug.log';
			// 		file_put_contents(
			// 			$log_file, 
			// 			date('[Y-m-d H:i:s] ') . json_encode($log_data) . PHP_EOL,
			// 			FILE_APPEND
			// 		);
			// 	}
			// }
