...
class Query_API extends \WP_REST_Controller {
	...
	public function get_dynamic_query( $request ) {
		...
		// debug
		// error_log( 'Query API: ' . json_encode( $args ) ) ;

		if ( 'posts' === $query_type ) {
			if ( $body['queryMake'] ) {
				// Check if this is a search query and if Relevanssi is active
				if ( isset( $args['s'] ) && ! empty( $args['s'] ) && function_exists( 'relevanssi_do_query' ) ) {
					$query_prep = new \WP_Query( $args );
					// Pass the query to Relevanssi
					relevanssi_do_query( $query_prep );
					
					// // Log Relevanssi results for debugging
					// if ( defined('WP_DEBUG') && WP_DEBUG ) {
					// 	$search_term = sanitize_text_field( $args['s'] );
					// 	$log_file = WP_CONTENT_DIR . '/relevanssi_api_debug.log';
					// 	$post_ids = array();
					// 	$post_titles = array();
						
					// 	if ( !empty( $query_prep->posts ) ) {
					// 		foreach ( $query_prep->posts as $post ) {
					// 			$post_ids[] = $post->ID;
					...
					// 		'found_posts' => $query_prep->found_posts,
					// 		'post_count' => $query_prep->post_count,
					// 		'post_ids' => $post_ids,
					// 		'post_titles' => $post_titles,
					// 		'query_vars' => $query_prep->query_vars,
					// 		'source' => 'api'
					// 	);
						
					// 	error_log( 'RELEVANSSI API SEARCH: ' . json_encode($log_data) . PHP_EOL );
					// }
